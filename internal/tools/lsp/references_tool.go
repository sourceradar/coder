...
func NewReferencesTool(manager *lsp.Manager) *tools.Tool {
	return &tools.Tool{
		...
		Explain: func(input map[string]any) tools.ExplainResult {
			...
			title := fmt.Sprintf("References(%s:%d:%d)", filePath, int(line), int(character))

			content := fmt.Sprintf("Will find all references to the symbol at %s:%d:%d",
				filePath, int(line), int(character))

			if hasIncludeDeclaration {
				if includeDeclaration {
					content += " (including declaration)"
				} else {
					content += " (excluding declaration)"
				}
			}
